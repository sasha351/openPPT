#!/usr/bin/env python3
"""Convert a Markdown outline to a .pptx directly, no Open WebUI involved.
...
Usage:
    python outline_to_pptx.py outline.md
    python outline_to_pptx.py outline.md -o deck.pptx -t brand.pptx
    pbpaste | python outline_to_pptx.py -    # read the outline from stdin

See README.md's "Outline format" section for the Markdown grammar.
"""
import argparse
import sys
...
def main():
    ...
    ap.add_argument("outline", help="path to a Markdown outline file, or - to read stdin")
    ...
    if not slides:
        sys.exit(
            "no slide-shaped content found — expected a '# Slide Title' heading "
            "per slide, with '-' bullets under it"
        )
